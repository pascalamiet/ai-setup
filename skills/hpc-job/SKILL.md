...
3. HPC project path — the absolute path to your project on the cluster.
   UNIGE HPC path structure: /home/users/a/USERNAME/PROJECT_NAME
   e.g. "/home/users/a/amiet/tst-llm"
   (This is added as a `cd` before your command so all relative paths work)

4. Which software modules do you need to load?
   e.g. "Python/3.11.3-GCCcore-12.3.0", "R/4.3.2-foss-2023a"
   (Leave blank if you will use conda or a venv — add activation line manually)

5. Partition — choose one:
...
6. Time limit (HH:MM:SS)  [00:15:00]
...
11. Email notifications when job ends or fails?  [no]
...
| `__HPC_PROJECT_PATH__`   | Absolute HPC project path from Q3 (e.g. /home/users/a/amiet/proj) |
| `__PARTITION__`          | Partition from Q5                                                  |
| `__TIME__`               | Time limit from Q6                                                 |
| `__NTASKS__`             | 1 for single/multi-threaded; N for MPI                             |
| `__CPUS__`               | N for multi-threaded; 1 for single-threaded and MPI                |
| `__MEM_MB__`             | Memory per CPU from Q9                                             |
| `__MODULE__`             | Module(s) from Q4, or remove the `module load` line if blank       |
